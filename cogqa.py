...
from utils import find_start_end_before_tokenized, judge_question_type, fuzzy_retrieve, get_context_fullwiki


# from line_profiler import LineProfiler

def cognitive_graph_propagate(tokenizer, data: 'Json eval(Context as pool)', model1, model2, device,
                              setting: 'distractor / fullwiki' = 'fullwiki', max_new_nodes=5):
...
def main(BERT_MODEL='bert-base-uncased', model_file='./models/bert-base-uncased.bin',
         data_file='./hotpot_dev_fullwiki_v1.json', max_new_nodes=5):
    ...
    with torch.no_grad():
        for data in tqdm(dataset):
            gold, ans, graph_ret, ans_nodes = cognitive_graph_propagate(tokenizer, data, model1, model2, device,
                                                                        setting=setting, max_new_nodes=max_new_nodes)
...
if __name__ == "__main__":
    ...
    assert 'bert-base-uncased' in ppb.modeling.PRETRAINED_MODEL_ARCHIVE_MAP
    fire.Fire(main)
